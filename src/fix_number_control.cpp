...
void FixNumberControl::insert_atom()
{
  ...
  while (ninserted < nparticles) {
	  ...
	  while (attempt < maxattempt) {
		...
		double temp = T0;
		double factor = 0.0;
		if(mode == ATOM) factor = sqrt(temp*force->boltz/atom->mass[ntype]*force->ftm2v);
		else
		{
			factor = sqrt(temp*force->boltz/onemols[imol]->masstotal*force->ftm2v);
		}
		if(domain->dimension == 3) {
			vnew[0] = random->gaussian();
			vnew[1] = random->gaussian();
			vnew[2] = random->gaussian();
			vnew[0] *= factor;
			vnew[1] *= factor;
			vnew[2] *= factor;
		...
		*/
		...
		for (m = 0; m < natom; m++) {
		  ...
		  if (flag) {
			...
			//atom->vest[n][0] = vnew[0];
			//atom->vest[n][1] = vnew[1];
			//atom->vest[n][2] = vnew[2];
...
//  double **v = atom->vest;
  ...
  for (i = nlocal-1; i >= 0; i--) {
    if (mark[i]) {
	  ...
	  // record momentum change due to deletion
	  int id = i;
	  double imass = atom->mass[atom->type[id]];
