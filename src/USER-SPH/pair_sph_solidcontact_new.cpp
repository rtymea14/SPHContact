...
#include "math.h"
#include "stdlib.h"
#include "pair_sph_solidcontact_new.h"
...
PairSPHSolidContactNew::PairSPHSolidContactNew(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
}
...
PairSPHSolidContactNew::~PairSPHSolidContactNew() {
  if (allocated) {
...
void PairSPHSolidContactNew::init_style() {
...
void PairSPHSolidContactNew::compute(int eflag, int vflag) {
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
	  ...
	  double rsqc = maxr*maxr + rav*rav; //4.0*maxr*maxr;  
	...
	if (ndim==2) {
		...
		double theta = abs(atan2(sin,cos));
		
	//	printf("theta = %g, cgix = %g, cgiy = %g, cgjx= %g, cgjy = %g, nix = %g, niy = %g, njx = %g, njy = %g\n",theta, cg[i][0], cg[i][1], cg[j][0], cg[j][1], ni[0], ni[1], nj[0], nj[1]);
		//Use the absolute value of theta
		...
		//printf("pn = %g, pnDot = %g, navx = %g, navy = %g, i = %d, j = %d\n", pnav, pnavDot, nav[0], nav[1], i, j);
		if (pnav < 0.0){
			pnav = 0.0;
			pnavDot = 0.0;
		}
		...
		double area = 2*pi*radius[i]; 
		//double area = 100*pi*radius[i]; 
		
		double fx = (lambda1*pnavDot + lambda2*pnav)*nav[0]*wij*area*Vj;
		double fy = (lambda1*pnavDot + lambda2*pnav)*nav[1]*wij*area*Vj;
		//printf("fx = %g, fy = %g, pn = %g, pnDot = %g\n", fx, fy, pnav, pnavDot);
		f[i][0] += fx;
		f[i][1] += fy;

		atom->de[i] -= (fx*vxtmp + fy*vytmp);

		//printf("theta = %g, thetac = %g, rijDotnav = %g, nav[0] = %g, nav[1] = %g, delvx = %g, delvy = %g, nix = %g, niy = %g, njx = %g, njy = %g\n",theta, thetac, rijDotnav, nav[0], nav[1], delvx, delvy, ni[0], ni[1], nj[0], nj[1]);
		//printf("fx = %g, fy = %g, lambda1 = %g, lambda2 = %g, area = %g, pn = %g, pnDot = %g, wij = %g, Vj = %g\n", fx, fy, lambda1, lambda2, area, pnav, pnavDot, wij, Vj);
...
*/		
		...
		double lambda1 = rho[j]*csound[j]*rho[i]*csound[i]/(rho[j]*csound[j] + rho[i]*csound[i]);
		double lambda2 = Ei*Ej/(Ei+Ej)/d0;
		double area = 4*pi*radius[i]*radius[i]; //for sphere
...
void PairSPHSolidContactNew::allocate() {
  allocated = 1;
  int n = atom->ntypes;
...
void PairSPHSolidContactNew::settings(int narg, char **arg) {
  if (narg != 0)
    error->all(FLERR,
...
void PairSPHSolidContactNew::coeff(int narg, char **arg) {
  if (narg != 3+5)
...
double PairSPHSolidContactNew::init_one(int i, int j) {
...
double PairSPHSolidContactNew::single(int i, int j, int itype, int jtype,
