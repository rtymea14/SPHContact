...
#include "math.h"
#include "stdlib.h"
#include "pair_sph_colorgradient_new.h"
...
PairSPHColorGradientNew::PairSPHColorGradientNew(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
...
PairSPHColorGradientNew::~PairSPHColorGradientNew() {
  if (allocated) {
...
void PairSPHColorGradientNew::init_style() {
...
void PairSPHColorGradientNew::compute(int eflag, int vflag) {
...
void PairSPHColorGradientNew::allocate() {
  allocated = 1;
  int n = atom->ntypes;
...
void PairSPHColorGradientNew::settings(int narg, char **arg) {
  if (narg != 1)
    error->all(FLERR,
...
void PairSPHColorGradientNew::coeff(int narg, char **arg) {
  if (narg != 4)
...
double PairSPHColorGradientNew::init_one(int i, int j) {
  if (setflag[i][j] == 0) {
...
double PairSPHColorGradientNew::single(int i, int j, int itype, int jtype, double rsq,
...
int PairSPHColorGradientNew::pack_comm(int n, int *list, double *buf, int pbc_flag,
    int *pbc) {
  int i, j, m;
  double **colorgradient = atom->colorgradient;
...
void PairSPHColorGradientNew::unpack_comm(int n, int first, double *buf) {
  int i, m, last;
  double **colorgradient = atom->colorgradient;
