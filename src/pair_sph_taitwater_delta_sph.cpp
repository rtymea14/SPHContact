...
#include "math.h"
#include "stdlib.h"
#include "pair_sph_taitwater_delta_sph.h"
#include "atom.h"
#include "force.h"
#include "comm.h"
...
PairSPHTaitwaterDeltaSph::PairSPHTaitwaterDeltaSph(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
...
PairSPHTaitwaterDeltaSph::~PairSPHTaitwaterDeltaSph() {
  if (allocated) {
...
void PairSPHTaitwaterDeltaSph::init_style() {
...
void PairSPHTaitwaterDeltaSph::compute(int eflag, int vflag) {
...
void PairSPHTaitwaterDeltaSph::allocate() {
  allocated = 1;
  int n = atom->ntypes;
...
void PairSPHTaitwaterDeltaSph::settings(int narg, char **arg) {
  if (narg != 0)
    error->all(FLERR,
        "Illegal number of setting arguments for pair_style sph/taitwater/delta/sph");
}
...
void PairSPHTaitwaterDeltaSph::coeff(int narg, char **arg) {
  if (narg != 9)
    error->all(FLERR,
        "Incorrect args for pair_style sph/taitwater/delta/sph coefficients");
  if (!allocated)
    allocate();
...
double PairSPHTaitwaterDeltaSph::init_one(int i, int j) {
...
double PairSPHTaitwaterDeltaSph::single(int i, int j, int itype, int jtype,
